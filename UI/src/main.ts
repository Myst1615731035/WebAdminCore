...
const initConfig = {
	install: (app) => {
		app.config.globalProperties._config = _config;
	},
};
...
app.use(initConfig).use(middleware).use(authentic).use(store).use(router).use(element).use(vxetable).use(utils).use(packages).mount('#app');
