...
namespace WebUtils
{
    ...
    {
        private static readonly string key = "IVNB69VF";
        private static readonly string iv = "rqCJPRcu";
        ...
        public static string Encrypt(string data)
        {
            ...
            try
            {
                byte[] byKey = Encoding.Latin1.GetBytes(key);
                byte[] byIV = Encoding.Latin1.GetBytes(iv);
                byte[] byData = Encoding.Latin1.GetBytes(data);
        ...
        public static string Decrypt(string data)
        {
            ...
            try
            {
                ...
                MemoryStream memoryStream = new MemoryStream();
                CryptoStream cryptoStream = new CryptoStream(memoryStream, descryptoServiceProvider.CreateDecryptor(byKey, byIV), CryptoStreamMode.Write);
                
                cryptoStream.Write(byData, 0, byData.Length);
                cryptoStream.FlushFinalBlock();
                result = Encoding.Latin1.GetString(memoryStream.ToArray());
            }
            catch
            {
                result = null;
