...
namespace WebExtention.Middlewares
{
    ...
    public static class AllServicesMiddleware
    {
        public static void UseAllServicesMiddle(this IApplicationBuilder app, IServiceCollection _services)
        {
            ...
            //List<Limit> tsDIAutofac = new List<Limit>();
            //tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, "WebExtention.Core.Services.dll")).GetTypes().ToList());
            //tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, "WebExtention.Core.Repository.dll")).GetTypes().ToList());
