...
namespace WebExtention.Middlewares
{
    ...
    public class RecordAccessLogsMiddleware
    {
        ...
        {
            if (AppConfig.Get("Middleware", "RecordAccessLogs", "Enable").ObjToBool())
            {
                var api = context.Request.Path.ObjToString().TrimEnd('/').ToLower();
                var ignoreApis = AppConfig.Get("Middleware", "RecordAccessLogs", "IgnoreApis");
                ...
                if (api.Contains("api") && !ignoreApis.Contains(api))
                {
                    ...
                    context.Response.OnCompleted(() =>
                    {
                        ...
                        LogHelper.WriteLog("RecordAccessLogs", requestInfo);
